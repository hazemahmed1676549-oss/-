...
function initApp(){
  ...
  buildSurahList();buildTafsirList();buildJuzGrid();buildHadithFilters();buildHadithList();buildAzkarCategories();buildKhatma();buildPrayerTimes();buildTasbih();buildStories();setDailyContent();buildLangList();buildReciters();
  
  const audio = document.getElementById('main-audio');
  audio.addEventListener('error', () => {
    if(audio.src) showToast(i18n[state.lang] ? i18n[state.lang].error_audio : 'Audio file currently unavailable');
  });
}
...
async function openTafsirSurah(id){const s=SURAHS.find(x=>x.id===id);document.getElementById('tafsir-title-bar').textContent=s.name;const content=document.getElementById('tafsir-content');content.innerHTML='<div style="text-align:center;padding:40px"><i class="fas fa-spinner fa-spin" style="font-size:40px;color:var(--gold)"></i></div>';navigate('tafsir-view');try{
  // Fetching both Quran text and Tafsir together
  const res=await fetch(`https://api.alquran.cloud/v1/surah/${id}/editions/quran-uthmani,ar.muyassar`);
  const data=await res.json();
  let html='<div class="ayah-container">';
  data.data[0].ayahs.forEach((a, i) => {
    html += `<div style="margin-bottom:20px"><span class="ayah-num">${i+1}</span> <span class="ayah" style="font-size:20px">${a.text}</span><p style="margin-top:10px;font-family:'Cairo';background:var(--bg);padding:10px;border-radius:8px;border-right:4px solid var(--primary-l)"><strong>التفسير:</strong> ${data.data[1].ayahs[i].text}</p></div>`;
  });
  html+='</div>';
  content.innerHTML=html;
}catch(e){content.innerHTML='<p style="text-align:center">تعذر تحميل التفسير.</p>';}}
...
function toggleHadithFav(btn, id) { 
  const idx = state.hadithFavorites.indexOf(id);
  if (idx > -1) { state.hadithFavorites.splice(idx, 1); btn.classList.remove('active'); showToast('تمت الإزالة من المفضلة'); } 
  else { state.hadithFavorites.push(id); btn.classList.add('active'); showToast('تمت الإضافة للمفضلة ❤️'); } 
  localStorage.setItem('hidaya_hadith_fav', JSON.stringify(state.hadithFavorites)); 
}
...
function buildQibla() {
  const c = document.getElementById('qibla-content');
  const t = i18n[state.lang] || i18n.en;
  c.innerHTML = `<div style="text-align:center;padding:40px"><i class="fas fa-spinner fa-spin" style="font-size:40px;color:var(--gold)"></i><p style="margin-top:10px">${t.enable_location}</p></div>`;
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(pos => {
      ...
      const lambdaK = (qiblaLon * Math.PI) / 180.0;
      ...
      let qiblaAngle = Math.round(psi);
      if (qiblaAngle < 0) qiblaAngle += 360;
      
      c.innerHTML = `
        <div style="background:var(--card);border:1px solid var(--border);border-radius:20px;padding:30px;display:inline-block">
          <div class="compass">
            <div class="compass-marks">
              <span class="n">N</span><span class="s">S</span><span class="e">E</span><span class="w">W</span>
            </div>
            <div id="needle" class="compass-needle" style="transform: rotate(${qiblaAngle}deg)"></div>
            <div class="compass-center"><i class="fas fa-kaaba"></i></div>
          </div>
          <h3 style="margin-top:20px;color:var(--primary)">${t.qibla_angle}: <span id="qibla-angle">${qiblaAngle}°</span></h3>
          <p style="color:var(--muted);font-size:14px">${t.qibla_instruction}</p>
        </div>
      `;
      ...
      if (typeof DeviceOrientationEvent !== 'undefined' && typeof DeviceOrientationEvent.requestPermission === 'function') {
        ...
        window.addEventListener('deviceorientationabsolute', (e) => handleOrientation(e, qiblaAngle), true);
      }
    }, () => {
      c.innerHTML = `<p style="text-align:center">${t.location_denied}</p>`;
    });
  } else {
    c.innerHTML = `<p style="text-align:center">${t.location_denied}</p>`;
  }
...
function handleOrientation(event, qiblaAngle) {
  ...
  if (event.webkitCompassHeading) alpha = 360 - event.webkitCompassHeading;
  if (alpha !== null && alpha !== undefined) {
    const needle = document.getElementById('needle');
    if (needle) {
      // Adjust Qibla needle based on compass heading
      needle.style.transform = `rotate(${qiblaAngle - alpha}deg)`;
    }
  }
}
...
function buildAudioSurahList(filter = '') {
  ...
  SURAHS.filter(s => !filter || s.name.includes(filter) || String(s.id).includes(filter)).forEach(s => {
    ...
    div.innerHTML = `<div class="surah-num">${s.id}</div><div class="surah-info"><h4>${s.name}</h4><div class="surah-meta"><span>${s.verses} آية</span><span class="surah-type ${s.type==='مكية'?'makki':'madani'}">${s.type}</span></div></div><i class="fas fa-play" style="color:var(--gold);font-size:18px"></i>`;
